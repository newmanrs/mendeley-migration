...
def process_pdf(paper):
    ...
    for entryname in bib.entries:  #Iterable, one element
        ...
        # Safer to make our own cite keys - override the bibtex file
        citekey = slugify.slugify(f"{name} {year}", separator='-')
        ...
        with open(f"outbox/{filename}.bib",'w') as f:
            bibstring = bib.to_string('bibtex')
            split = bibstring.splitlines()
            slug = slugify.slugify(split[0], lowercase=False).split('-')
            doctype = slug[0]
            split[0] = '@' + doctype +'{' + citekey + ','
            bibstring = '\n'.join(split)+'\n'
            f.write(bibstring)
        #shutil.move(paper, f"outbox/{filename}.pdf")

        shutil.copy(paper, f"outbox/{filename}.pdf")
